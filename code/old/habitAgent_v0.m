function results = habitAgent_v0(sched, agent, input)
...
for s = 1:sched.sessnum
    ...
    while t <= timeSteps
        ...
        a = fastrandsample(policy);    % action (1) null (2) lever
        ...
        if deval(t) == 0 && test(t) == 0
            p = p + alpha_pi*(policy - p); p = p./nansum(p);        % marginal update
        end
        ...
        switch sched.model % different models with different costs
            ...
            case 4  % yes action cost, yes complexity cost (adaptive)
                ...
                r = ~test(t)*double(x(t)==2)-acost*(a(t)==2);
                %rna=r;
                rpe = agent.beta*r - rho + V - V0 - cost;      % reward prediction error
        ...
        if test(t) == 0 % only update if not in test mode
            ...
            w = w + alpha_w*rpe;                       % weight update with value gradient
            ...
            theta(:,a) = theta(:,a) + (agent.lrate_theta)*rpe*g;      % policy weight update
