...
legend('VI1','VI2','VI5','VI10','VI20','Location','southeast')
legend('boxoff')
axis([0 1 0 5])
axis square
prettyplot
...
subplot 223; plot(la,VI.meanNumAct,'LineWidth', 1.5);xlabel('\lambda_a');ylabel('mean #A until reward');axis square;prettyplot; % mean number of actions till reward
% mean number of actions till reward
subplot 224; plot(la,VI.meanATO,'LineWidth', 1.5);xlabel('\lambda_a');ylabel('mean time A->O');axis square;prettyplot; % mean number of actions till reward
...
subplot 223; plot(la,FI.meanNumAct,'LineWidth', 1.5);xlabel('\lambda_a');ylabel('mean #A until reward');axis square;prettyplot; % mean number of actions till reward
subplot 224; plot(la,FI.meanATO,'LineWidth', 1.5);xlabel('\lambda_a');ylabel('mean time A->O');axis square;prettyplot; % mean number of actions till reward
...
figure;
subplot 222; plot(FI.waitTimeMean,VI.waitTimeMean,'.'); dline;xlabel('FI wait time');ylabel('VI wait time');prettyplot;% mean of the inter-outcome interval for diff action rates. this 1/lambda_o
subplot 223; plot(FI.meanNumAct,VI.meanNumAct,'.'); dline;xlabel('FI mean num actions');ylabel('VI mean number actions');prettyplot; % mean number of actions till reward
subplot 224; plot(FI.meanATO,VI.meanATO,'.'); dline;xlabel('FI mean of H_{O|A}');ylabel('VI mean of H_{O|A}');prettyplot; % mean number of actions till reward
%mean of the action to outcome distribtion (conditional entropy)
...
for i = 1:size(VI.I,1)
VI.dIdA(i,:) = gradient(VI.I(i,:))./gradient(la);
VI.dAdO(i,:) = gradient(la)./gradient(VI.lambda_o(i,:));
end
